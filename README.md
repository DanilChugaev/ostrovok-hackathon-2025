...
- Тестовые пользователи:
  - `user/user` - обычный пользователь, без возможности заполнять отчеты по поездкам, есть доступ к программе лояльности
  - `secret_guest/secret_guest` - секретный гость, есть доступ к заполнению отчета по поездкам, есть доступ к программе лояльности
